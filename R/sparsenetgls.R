...
#'Bühlmann penalized linear regression and the neighbourhood selection with
...
sparsenetgls <- function(responsedata,predictdata,nlambda=10,ndist=5,
                        method="lasso",lambda.min.ratio=0.00001)
{
    ...
    penalized_seq <- huge(covy,lambda.min.ratio=lambda.min.ratio,
                        nlambda=nlambda,
    scr=FALSE,cov.output = TRUE,method="glasso")
    PREC_seq=penalized_seq$icov; lambdav=penalized_seq$lambda
    
    } else if (method=="mb") {
    penalized_seq <- huge(Y1,lambda.min.ratio=lambda.min.ratio,nlambda=nlambda,
                            scr=TRUE,sym="and",method="mb")
    PREC_seq <- convert_prec(adjm=penalized_seq$path,nlambda=nlambda,
                            sample_var=covy,Y=Y1,p=p)
    lambdav=penalized_seq$lambda
    
    } else if (method=="lasso") {
    fitlasso <-lassoglmnet(Y1,nlambda=nlambda,alpha=1); lambdav=fitlasso$lambda
    #covert array to a list
    PREC_seq <- lapply(seq(nlambda),function (x)
                beta_to_omega(Beta=fitlasso$coef_glmnet,
                resid=fitlasso$resid_glmnet,pathnumber=nlambda)[,,x])
    
    } else if (method=="elastic") {
    fitlasso_enet <- lassoglmnet(Y1,nlambda=nlambda,alpha=0.5)
    lambdav=fitlasso_enet$lambda
    PREC_seq <- lapply(seq(nlambda),function (x)
                beta_to_omega(Beta=fitlasso_enet$coef_glmnet,
                resid=fitlasso_enet$resid_glmnet,pathnumber=nlambda)[,,x])
    }

#Derive the path sandwitch parameter and its variance
#using the chosen cov of tuning parameter d
    #dist_adj <- dist_tune(covy=covy,covstart=penalized_pcor,ndist=ndist,p=p)
    dist_adj <- dist_tune(covy=covy,covstart=PREC_seq[[round(nlambda/2,0)]],
                        ndist=ndist,p=p)
    ...
    B[,,power,lambda] <- t(X)%*%V_1_kron%*%cov_adj_kron%*%(V_1_kron%*%X)
    power=power+1
        }
}
...
for ( lambda in seq_len(nlambda)){
    ...
    bic[s,lambda] <- -Matrix::determinant(PREC_seq[[lambda]])$modulus
    +t(Y-X%*%beta[,lambda])%*%V_1_kron%*%(Y-X%*%beta[,lambda])
    +density(PREC_seq[[lambda]],p)*log(n)/n
    aic[s,lambda] <- -Matrix::determinant(PREC_seq[[lambda]])$modulus
    +t(Y-X%*%beta[,lambda])%*%V_1_kron%*%(Y-X%*%beta[,lambda])
    +density(PREC_seq[[lambda]],p)/n
    }
}
return(list(beta=beta,covBeta=covBeta[,,seq_len(dist_adj$power),],
        lambda=lambdav,power=power,bic=bic,aic=aic,PREC_seq=PREC_seq))}
